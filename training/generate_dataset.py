...
def build(rng: random.Random, count: int) -> list[dict]:
    ...
    def add(domain: str, messages: list[dict]) -> None:
        nonlocal n
        # Produce natural prompt/opener variants instead of asking the model to memorise
        # one exact spelling of each task.  The prior generator requested 4,000 rows but
        # deduplication collapsed it to 69, which was far too little to adapt tool use.
        prompt_forms = (
        ...
        for message in messages:
            if message.get("role") == "user":
                ...
                form = prompt_forms[variant % len(prompt_forms)]
                message["content"] = form.format(original)
                break
        for message in messages:
