...
def main() -> int:
    ...
    if run(f"git -C {LLAMA} init") != 0 or run(
        f"git -C {LLAMA} fetch --depth 1 https://github.com/ggml-org/llama.cpp {LLAMA_REF}"
    ) != 0 or run(f"git -C {LLAMA} checkout --detach FETCH_HEAD") != 0:
    ...
    # Build the quantiser from the same pinned source as the converter. A moving
    # "latest" binary is faster, but it makes the release impossible to reproduce.
    if source_quantize(F16, OUT):
        ...
        with open(OUT, "rb") as handle:
            for chunk in iter(lambda: handle.read(8 * 1024 * 1024), b""):
                digest.update(chunk)
        manifest = {
            "artifact": os.path.basename(OUT), "base_model": "ibm-granite/granite-4.1-3b",
            "parameters": "3B", "quantization": "Q4_K_M", "format": "GGUF",
            "bytes": os.path.getsize(OUT), "sha256": digest.hexdigest(),
