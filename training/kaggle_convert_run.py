...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as raw:
        staging = Path(raw)
        shutil.copy2(Path(__file__).with_name("convert_kaggle.py"), staging / "kilobyte-convert.py")
        (staging / "kernel-metadata.json").write_text(json.dumps({
            "id": ref, "title": "Kilobyte GGUF Convert", "code_file": "kilobyte-convert.py",
            "language": "python", "kernel_type": "script", "enable_gpu": False,
            "enable_internet": True, "dataset_sources": [], "model_sources": [],
            "competition_sources": [], "kernel_sources": [args.training_kernel],
        }, indent=2))
    ...
    if "complete" not in state.lower():
        print(f"conversion did not complete cleanly: {state}")
        return 1
