        ...
        try:
            while raw := await reader.readline():
                ...
                if kind == "session":
                    ...
                    if not at_line_start:
                        sys.stdout.write("\n")
                    # The model starts a new assistant continuation after every tool
                    # result. Reset leading-whitespace state so its habitual \n\n prefix
                    # cannot become spacer rows between live work and the final answer.
                    markdown = MarkdownStream()
