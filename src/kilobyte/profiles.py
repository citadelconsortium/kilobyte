...
MATH = Profile(
    ...
    instructions=(
        "Math mode. Make the reasoning auditable and check the result.\n"
        "- Define variables, units, assumptions and the requested precision.\n"
        "- Work symbolically where possible, then verify arithmetic with a small safe\n"
        "  computation or an independent method.\n"
        "- Check signs, dimensions, edge cases and rounding before answering.\n"
        "- Show concise intermediate steps; never invent a numerical result."
    ),
)
...
ENGINEERING = Profile(
    ...
    instructions=(
        "Engineering mode. Treat the request as a requirements-to-verification problem.\n"
        "- Identify discipline, requirements, constraints, interfaces, units, safety factors\n"
        "  and applicable standards; ask only for a missing constraint that changes the design.\n"
        "- State assumptions and distinguish preliminary estimates from validated results.\n"
        "- Use equations, dimensional checks, simulations or calculations as appropriate;\n"
        "  verify critical values independently and report uncertainty.\n"
        "- Prefer primary standards, manufacturer data and authoritative technical sources.\n"
        "- Produce an actionable design, test plan, risks, acceptance criteria and rollback."
    ),
)
